...
class CI_DB_active_record extends CI_DB_driver {
     ...
     * @return	void
     */
    public function set_id($table, $key, $type, $length) {
        $new_id = '';
        if ($type == 'no_prefix') {
            $prefix = '';
        } else if ($type == 'date_prefix') {
            $prefix = date('ymd');
        }

        if ($this->_get_primary($table, $key, $prefix)->count_all_results() > 0) {
            $this->order_by($key, 'desc');
            $last_id = $this->_get_primary($table, $key, $prefix)->get()->row()->{$key};
            $new_id = $last_id + 1;
            $next_id = str_pad($new_id, $length, "0", STR_PAD_LEFT);
        } else {
            $length -= strlen($prefix);
            $new_id = str_pad('1', $length, "0", STR_PAD_LEFT);
            $next_id = $prefix . $new_id;
        }

        $this->set($key, $next_id);

        return $next_id;
    }

    private function _get_primary($table, $key, $prefix = '') {
        $this->from($table);
        if (!empty($prefix))
            $this->like($key, $prefix, 'after');
        $this->limit(1);
        return $this;
    }
     ...
     *      $condition = array(
     *                      'AND' => array(
     *                          'field_name1' => $keyword1
     *                      )
     *                      'OR' => array(
     *                          'field_name2' => $keyword2
     *                      )
     *                   );
     * Enjoyed!
     */
    public function where_condition($condition = array()) {
        if (is_array($condition)) {
            if (!isset($condition['AND']) && !isset($condition['OR'])) {
                if (count($condition) > 0)
                    $this->where($condition);
            } else {
                if (isset($condition['AND']) && is_array($condition['AND']) && count($condition['AND']))
                    $this->where($condition['AND']);
                if (isset($condition['OR']) && is_array($condition['OR']) && count($condition['OR']))
                    $this->or_where($condition['OR']);
            }
        }
    }
